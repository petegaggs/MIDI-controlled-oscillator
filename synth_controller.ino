...
#define PWM_DAC_MULTIPLIER 0.24 // used for mk2 h/w
...
void setup() {
  ...
  // timer 1 interrupts.
  TIMSK1 = _BV(TOIE1);
  // timer 2 fast pwm, no prescaling, non inverting mode channel B
...
SIGNAL(TIMER1_OVF_vect)
{
  ...
  if (oddEvenIntCnt) {
    PORTB |= 0x10; // set PB4 (arduino pin 12)
    oddEvenIntCnt = false;
    delayMicroseconds(10); // give a bit of time to discharge capacitor
    PORTB &= ~0x10; // clear PB1 (arduino pin 9)    
  } else {
    oddEvenIntCnt = true;
  }
}
